...
def store_h5_dict(path, data_dict, compression="lzf", **ds_kwargs):
    ...
    for k, v in data_dict.items():
        ...
        # print(k,max_shape)
        hf.create_dataset(k, data=v, compression=compression, maxshape=max_shape, **ds_kwargs)
    hf.close()

# appends to an existing h5
def store_h5_dict_a(path, data_dict, compression="lzf", **ds_kwargs):
    hf = h5py.File(path, 'a')
    for k, v in data_dict.items():
        # hf.require_dataset(k, compression=compression, **ds_kwargs)
        old_shape = hf[k].shape
        # print("OLD SHAPE", old_shape)
        hf[k].resize(old_shape[0] + np.array(v).shape[0], axis=0)
